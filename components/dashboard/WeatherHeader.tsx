...
export function WeatherHeader({ weather }: Props) {
  ...
  return (
    <div
      className="relative rounded-2xl overflow-hidden border border-white/7"
    ...
    >
      ...
      {/* ══════════════════════════════════════════════════════════
          DESKTOP: satu baris — cuaca kiri, jam kanan
          MOBILE:  dua baris — cuaca atas, jam + sapaan bawah
          ══════════════════════════════════════════════════════════ */}
      <div className="relative z-10 px-5 py-5 md:px-8 md:py-6">

        {/* Baris atas: selalu ada di desktop & mobile */}
        <div className="flex items-center gap-6 md:gap-10">
          ...
          <div className="flex items-center gap-6 md:gap-10 flex-1 min-w-0">
            ...
            <div className="shrink-0">
              ...
              <p className="text-[11px] text-white/30 mb-3">{dateStr}</p>
              {weather ? (
                <div className="flex items-end gap-2">
                  ...
                  <div className="pb-1">
                    <span className="text-xl md:text-2xl text-white/30 font-light">°C</span>
                    <p className="text-sm text-white/45 mt-0.5 capitalize">{weather.description}</p>
                  </div>
                </div>
              ) : (
            ...
            {/* Divider — hanya desktop */}
            <div className="hidden md:block h-20 w-px bg-white/6 shrink-0" />

            {/* Stats — desktop: tampil di sini (row), mobile: tampil di bawah cuaca */}
            {weather && (
          ...
          {/* ── Jam — hanya desktop (kanan) ──────────────── */}
          <div className="hidden md:block text-right shrink-0 pl-10 border-l border-white/6 ml-4">
            <p className="text-sm text-white/40 font-light mb-1">{emoji}&nbsp;{greetText}</p>
        ...
        {/* ── Mobile only: stats + jam/sapaan di bawah ─── */}
        <div className="md:hidden mt-4 pt-4 border-t border-white/6 space-y-3">
          {/* Stats row */}
          {weather && (
            <div className="flex gap-5">
              <StatSm label="Terasa" value={`${weather.feelsLike.toFixed(1)}°C`} />
              <StatSm label="Lembap" value={`${weather.humidity}%`} />
              <StatSm label="Angin"  value={`${weather.windSpeed} m/s`} />
            </div>
          )}

          {/* Jam + sapaan — lebih kecil, di bawah cuaca */}
          <div className="flex items-center justify-between">
            <p className="text-xs text-white/30">{emoji}&nbsp;{greetText}</p>
            <p className="text-2xl font-bold text-white font-mono tracking-tight tabular-nums">
              {clockStr}
            </p>
          </div>
        </div>
